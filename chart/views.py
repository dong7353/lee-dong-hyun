...
def chart_teacher(request):
    ...
    for t in teacher_list:
        ...
        try:
            # 해당 선생님이 강의하는 강의 번호(관계키 이용)
            l_no = lecture_tbl.objects.get(t_no_id=t.t_no).l_no
            try:
                # 해당 선생님이 강의하는 학생들의 수 카운트
                student_count = training_tbl.objects.filter(l_no_id=l_no).count()
