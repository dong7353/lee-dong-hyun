...
def index(request):
    ...
    day = timezone.now()
    context = {'day' : day, 'teacher_list' : teacher_list}
...
# 선생님 상세
@login_required(login_url='admin:login_admin')
def teacher_detail(request, teacher_tbl_t_no):
    """
    선생님 상세
    """
...
def login_admin(request):
    if request.method == "POST":
        ...
        # 정보가 틀렸을 시 새로운 스크립트에 메세지를 띄우고 확인 시 원래창으로 돌아오기
            ...
            return HttpResponse('<script type="text/javascript"> alert("다시 입력 해 주세요"); history.back();</script>' )
